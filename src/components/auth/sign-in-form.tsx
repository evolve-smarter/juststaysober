...
export function SignInForm({ isSignup = false }: { isSignup?: boolean }) {
  ...
  if (sent) {
    return (
      <div className="text-center py-4">
        ...
        <p className="text-gray-400 text-sm mt-1">We sent a magic link to {email}</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 bg-white text-gray-900 hover:bg-gray-100 px-4 py-3 rounded-xl font-medium transition-colors"
      >
      ...
      <form onSubmit={handleEmail} className="space-y-3">
        ...
        >
          {loading ? 'Sending...' : `${isSignup ? 'Sign up' : 'Sign in'} with email`}
        </button>
      </form>
